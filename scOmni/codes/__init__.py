...
def batch_integration(adata, batch_key, method=["liger"]):
    ...
    def run_harmony(adata, batch):
        ...
        adata.obsm["X_harmony"] = adata.obsm["X_pca_harmony"]  # Harmony results are stored in X_pca_harmony
        return adata
    ...
    def run_liger(adata, batch):
        ...
        # Ensure dense matrix before converting to sparse
        ...
        adata.obsm["X_liger"] = adata.obsm["X_pca_liger"]  # LIGER results are stored in X_pca_liger
    ...
    def run_scvi(adata, batch):
        ...
        if 'counts' not in adata.layers:
            # Get indices of current adata.var_names in raw.var_names
...
def celltype_annotation(adata, species='', tissue_type='', cancer_type='Normal', groupby='', method=["gpt4"], openai_api_key=None): 
    # Can be a single string or a list, such as ["gpt4", "cellmarker", "act"]
    ...
    def cellmarker_method(adata, species, tissue_type, cancer_type, groupby):
        ...
        marker = pd.read_excel("/root/CellAgent/scOmni/marker_database/Cell_marker_Seq.xlsx")
        filtered_df = marker[
    ...
    def act_method(adata, species, tissue_type, groupby):
        marker = pd.read_csv("/root/CellAgent/scOmni/marker_database/ACT.csv")
        filtered_df = marker[
            (marker['Species'] == species) &
...
def trajectory_top_k_methods(
    ...
    try:
        ...
        print("STDOUT:")
        print(result.stdout)
        methods = re.findall(r'"([^"]+)"', re.search(r'\[\d+\]\s+("[^"]+"(\s+)*)+', result.stdout).group()) if re.search(r'\[\d+\]\s+("[^"]+"(\s+)*)+', result.stdout) else []
...
# Trajectory Inference API - Direct one-step invocation
def trajectory_inference(adata, groupby=None, obsm_embedding=None, start_id=None, expected_topology=None, fixed_n_methods=3):
    """
    Perform trajectory inference using the top selected methods.

    Parameters:
    - adata: AnnData object containing the data.
    - groupby: Grouping key for clustering (optional).
    - obsm_embedding: Embedding key in `adata.obsm` (optional).
    - start_id: Starting cell ID for trajectory inference (optional).
    - expected_topology: Expected topology of the trajectory (optional).
    - fixed_n_methods: Number of methods to select for trajectory inference (default: 3).

    Returns:
    None: This function performs trajectory inference and saves the results.
    """
    def top_k_methods(
        adata,
        time_limit='20m',
        ...
        """
        Select the top k methods for trajectory inference.

        Parameters:
        - adata: AnnData object.
        - time_limit: Time limit for method selection.
        - memory_limit: Memory limit for method selection.
        - fixed_n_methods: Number of methods to select.
        - expected_topology: Expected topology of the trajectory.
        - start_id: Starting cell ID.
        - end_id: Ending cell ID.
        - groups_id: Grouping key.
        - PCA: PCA embedding (optional).
        - X_umap: UMAP embedding (optional).
    ...
    def run_method(method_name=None):
        """
        Run a specific trajectory inference method.
...
# Spatial domain (spatial clustering) API
def spatial_domain_identify(adata_st, n_domains=None, methods=None):
    ...
    script_paths = {
        'DeepST': '/data/cellana/spatial_analysis/scripts/DeepST_cluster.py',
        'stLearn': '/data/cellana/spatial_analysis/scripts/stLearn_cluster.py',
        'SEDR': '/data/cellana/spatial_analysis/scripts/SEDR_cluster.py',
    }
...
# Spatial clustering visualization API
def spatial_domain_plot(method=None):
    ...
    # If method is None, iterate through all available methods
    if method is None:
        methods = plots_paths.keys()
    else:
        methods = [method] if method in plots_paths else []  # Ensure method is in the dictionary

    # Iterate and display images, skip if the file does not exist
    for method in methods:
...
def stLearn_spatial_trajectory(adata, use_label="louvain_morphology", cluster=0):
    ...
    # Create TI_plot folder if it does not exist
    ...
    script_path = '/data/cellana/spatial_analysis/scripts/stLearn_spatial_trajectory.py'
...
def impute_method_run(adata_sc, adata_st, train_genes=None, predict_genes=None, value_genes=None, methods=['Tangram', 'gimVI', 'SpaGE']):
    ...
    script_path = "/data/cellana/single_cell_analysis/scripts/gene_imputation_script.py"
    try:
